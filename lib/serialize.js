 ...
 */
function build(srcPath, distPath) {
  ...
  var result = ['export const isSerialized = true;'];
  ...
  // 1. 配置文件 config.json
  //===============================================================
  var HANDLER_CONFIG_PATH = options.handlerConfigPath || path.join(srcPath, 'config.json');
  ...
  //===============================================================
  var HANDLE_MODULE_BASE_PATH = options.handleModuleBasePath || path.join(srcPath, 'handle_modules');
  ...
  if (fs.existsSync(HANDLE_MODULE_BASE_PATH)) {
    ...
    fsHandler.search.getAll(HANDLE_MODULE_BASE_PATH, { globs: ['*'] }).forEach(function (item) {
      // 获取各个 handle_module 中 config.json 的数据
      var curHandleModuleConfigName = '';
      ...
      if (item.isDirectory()) {
        ...
        var handleModuleRelativePath = path.join(curHandleModuleName, options.handleModuleConfigRelativePath || './config.json');
  ...
  // babel 之后的文件存储一份
  var data = babelD.babelCompile.compileByBabel(content);
